...
func _physics_process(_delta):
	if (chasePlayer):
		SetVelocity(player.global_position)
		LookAtTarget(player)
...
func CheckForBodies(body):
	if (body is PlayerCharacter):
		...
		return
	if (endOnCollisionOverride):
		if (!(body is EnemyProjectile) && !(body is EnemyController)):
			call_deferred("DeleteSelf")

func LookAtTarget(target: Node2D):
	self.look_at(target.global_position)
	self.rotation_degrees += rotationOffset

func DecreasePlayerSpeed(playerCharacter: PlayerCharacter):
	if (!playerCharacter.playerHealth.invulnerabilityActive):
		collisionSound.play()
		if (directDamage): playerCharacter.playerHealth.CheckForLoseHeart()
